...
OpenGLMeshBuffer::OpenGLMeshBuffer(const ModelData &data)
    :  vao_{
        []()-> GLuint { auto vao = GLuint{}; glCreateVertexArrays(NumVAOs{1}, &vao); return vao; }(), 
        [](auto vao) { glDeleteVertexArrays(NumVAOs{1}, &vao); } 

    }, vbo_{
        []()-> GLuint { auto vbo = GLuint{}; glCreateBuffers(NumVBOs{1}, &vbo); return vbo; }(), 
        [](auto vbo) { glDeleteBuffers(NumVBOs{1}, &vbo); }

    }, ebo_{
        []()-> GLuint { auto ebo = GLuint{}; glCreateBuffers(NumEBOs{1}, &ebo); return ebo; }(), 
        [](auto ebo) { glDeleteBuffers(NumEBOs{1}, &ebo); }

    }, vertex_stride_{sizeof(VertexData)}
{
    glNamedBufferStorage(vbo_, data.vertices.size_bytes(), data.vertices.data(), GL_STATIC_DRAW);
    glNamedBufferStorage(ebo_, data.indices.size_bytes(), data.indices.data(), GL_STATIC_DRAW);
    ...
    glVertexArrayAttribFormat(vao_, AttributeIndex{0}, Size{3}, GL_FLOAT, GL_FALSE, offsetof(VertexData, position));
