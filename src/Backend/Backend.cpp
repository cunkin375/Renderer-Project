...
namespace ShaderWatcher
{
    ...
    {
        g_shader_watcher =
            std::make_unique<DirectoryWatcher>(path,
                                               [](const DirectoryWatcher::FileEvent &event) -> void
                                               {
                                                   // this is fired on every event because vim never produces
                                                   // IN_MODIFY it should fire on every event, as an event is
                                                   // only meant to fire when a shader is modified NOTE:
                                                   // DirectoryWatcher might change, so come back here when
                                                   // that happens
